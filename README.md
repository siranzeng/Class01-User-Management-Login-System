...
| 漏洞 | 描述 | 修复方式 |
|------|------|----------|
| SSTI模板注入 | render_template_string 拼接用户输入，{{config}} 可泄露SECRET_KEY等配置 | 过滤 `{{`、`}}`、`{%`、`%}`、`{#`、`#}` 等 Jinja2 模板语法标记 |
...
**新增功能：**
- 密码修改（/change-password），支持用户修改自己的登录密码
- 个人中心页面增加修改密码表单（原密码、新密码、确认密码）

**修复漏洞：**

| 漏洞 | 描述 | 修复方式 |
|------|------|----------|
| CSRF跨站请求伪造 | /change-password 无 CSRF Token 校验，攻击者可构造恶意页面诱导用户改密码 | 增加 CSRF Token 校验，一次性 Token + compare_digest |
| 越权修改密码 | 任何登录用户可修改任意用户密码，未校验 username 归属 | 增加越权校验，仅允许修改当前登录用户密码 |
| 无原密码验证 | 修改密码不需要原密码，存在撞库后直接篡改风险 | 增加原密码验证，必须正确才能设置新密码 |

**修改文件：** app.py, templates/profile.html, README.md

---

### v2.5 — 动态页面加载功能 + 文件包含漏洞(LFI/RFI)修复
